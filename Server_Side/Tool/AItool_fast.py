# coding:utf-8
# Author:Mr Zhipan wang and Mr Zhongwu Wang, if you have any question, Email:1044625113@qq.com,Date:2024-6-2
import os
import onnxruntime
...
def model_predict(onnx_session, data, blocksize, pad_width, area, pro, isUseVote=0):
    ...
    if (row == blocksize + 2 * pad_width) & (col == blocksize + 2 * pad_width):
        ...
        data = normBlockImg(data, nodataValue=0)  
        ...
        if isUseVote == 0:
            ...
            imgy = backGroundMask(imgy, oridata, nodataValue=0)
        ...
        data = normBlockImg(oridata, nodataValue=0) 
        ...
        if isUseVote == 0:
            ...
            imgy_temp = backGroundMask(imgy_temp, oridata, nodataValue=0)
...
def getUnionRaster(inputChannel, onnx_model_Path, imgPath_t1, imgPath_t2, saveRasterPath,
    ...
    if datasetT1.RasterCount != datasetT2.RasterCount:
        ...
        dst_ds = driver.Create(saveRasterPath, Xsize_T1, Ysize_T1, 1, gdal.GDT_Byte, options=["TILED=YES", "COMPRESS=LZW", "BIGTIFF=YES"]) 
        ...
        for x in range(colT1_ul, colT1_lr, blocksize):
            ...
            for y in range(rowT1_ul, rowT1_lr, blocksize):
                ...
                if (not isBackGround(dataT1)) and (not isBackGround(dataT2)):  
                    ...
                    if (row == blocksize + 2 * pad_width) & (
                        ...
                        imgy = model_predict(resnet_session, bandcomImg, blocksize, pad_width, area, pro, isUseVote)
                        ...
                        imgy = model_predict(resnet_session, newdata, blocksize, pad_width, area, pro, isUseVote)
